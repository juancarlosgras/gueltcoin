...
#if defined(NDEBUG)
# error "Guelt cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1e8dab49f0e4fd56a9b123deb394bf7bf6d56099e042311969aaab2ab9452fcb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Guelt: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Guelt Signed Message:\n";
...
{
    // Guelt: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Guelt: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30; // Guelt: 1/2 minutes
static const int64 nTargetSpacing = 1 * 10; // Guelt: 10 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Guelt: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Guelt: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1e8dab49f0e4fd56a9b123deb394bf7bf6d56099e042311969aaab2ab9452fcb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "10/Oct/2017 Puigdemont pone en suspenso los efectos de la independencia para dialogar";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x77c9fe705fa9409fd4c685e431ad9117fcb923bf3a4a3564aeed976a9ee2622c"));
...
unsigned char pchMessageStart[4] = { 0xf3, 0xa2, 0xd5, 0xbf }; // Guelt: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GueltMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GueltMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GueltMiner(CWallet *pwallet)
{
    printf("GueltMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("guelt-miner");
    ...
    try { loop {
        ...
        printf("Running GueltMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GueltMiner, pwallet));
}
